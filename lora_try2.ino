...
const float SF = 7;  // Spreading factors 7 to 12
const float BW = 125000;  // Bandwidths in Hz: 125kHz, 250kHz, 500kHz

// Calculate the Time on Air in milliseconds
float calculateToA(uint8_t spreadingFactor, uint32_t bandwidth, uint16_t payloadLength) {
    ...
    const float codingRate = 5;  // 4/5 coding rate by default (4 data bits per 5 transmitted bits)
...
void loop()
{
  if (rf95.available())
  {
    ...
    if (rf95.recv(buf, &len))
    {
      ...
      float timeOnAir = calculateToA(SF, BW, len);
      Serial.print("Time on Air: ");
      Serial.print(timeOnAir);
      Serial.println(" ms");
      
      // Send a reply
      uint8_t data[] = "HelloButt!";
      rf95.send(data, sizeof(data));
      rf95.waitPacketSent();
      Serial.println("Sent a reply");
      
      // Calculate Time on Air for the reply message
      timeOnAir = calculateToA(SF, BW, sizeof(data));
